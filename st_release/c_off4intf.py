...
def c_off4intf(data_dir: str, id1: str, id2: str,
    ...
    if filter_strategy:
        ...
        medfilt_size = 15  # - Median filter Kernel Size
        medthre = 0.5  # - Median filter Threshold
        mask = median_filter_off(off_map, size=medfilt_size, thre=medthre)
        # - Set Outliers Mask borders equal to 1
        mask[:, 0:2] = 1
        mask[:, poff.npix - 2:] = 1
    ...
    if fill:
        ...
        while len(np.nonzero(np.isnan(ind_xoff)[0])) & \
                len(np.nonzero(np.isnan(ind_yoff)[0])):
            xoff_masked \
